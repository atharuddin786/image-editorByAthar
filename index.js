...
const loadImage = () => {
  ...
  previewImg.addEventListener("load", () => {
    resetFilterbtn.click(); //clicking reset button so the filter value reset if user select new image
...
rotateOptions.forEach((option) => {
  ...
  option.addEventListener("click", () => {
    if (option.id === "left") {
      rotate -= 90; //if clicked btn is left rotate,decreament rotate value by -90
    } else if (option.id === "right") {
      rotate += 90; //if clicked btn is right rotate,increament rotate value by -90
...
const saveImage = () => {
  console.log("save image button is clicked");
  //to download image
  ...
  link.click(); //click a tag to download image
};
